...
ghi_data <- ghi_data %>%
  ...
  # Extract longitude and latitude from the "location.coordinates" variable
  separate(location.coordinates, c("long", "lat"), sep = ",") %>% 
  mutate_at(
    vars(long, lat), 
    funs(
      str_replace(., "[\\(|\\)]", "") %>% str_replace("c", "") %>% as.numeric
    )
  ) %>%
...
# geographical coordinates and add a column indicating whether the ratio is High
# or Low
copd <- ghi_hrrp %>%
  select(lat, long, readm_ratio, name, measure_id) %>%
  filter(measure_id == "COPD", !is.na(lat), !is.na(long)) %>%
  mutate(ratio_lev = ifelse(readm_ratio >= 1, "Hi", "Lo") %>% as.factor)

# Create a spatial dataframe of the hospital coordinates and other values for 
# the map
copd_geo <- SpatialPointsDataFrame(
  cbind(copd$long, copd$lat), 
  data.frame(
    ratio_lev = copd$ratio_lev, readm_ratio = copd$readm_ratio,
    name = copd$name
  )
)

# Make a color palette of red for Hi ratios and navy for Lo ratios
cols <- colorFactor(c("red", "navy"), copd$ratio_lev)
